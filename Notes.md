...
- Issues left are: since delay(10): was removed in the for loop, I can't as easily control the speed of the servo; and the whole system works correctly when attached to my mac but when i unplug it and power it with a battery instead, the servo becomes jumpy and doesn't always turn correctly
...
## Day 2
- Added a function to output the correct data (current state and threshold distance) to the LCD screen and refreshed the screen every time it checks if either is changed
